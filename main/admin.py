...
class CourseAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ('Детали курса', {
            'fields': ('level', 'duration_weeks', 'lessons_count')
        }),
        ('Цена и акции', {
            'fields': ('price', 'old_price', 'is_free'),
            'classes': ('collapse',)
        }),
        ('Визуальное оформление', {
            'fields': ('icon_class', 'badge_text', 'badge_color'),
            'classes': ('collapse',)
        }),
        ('Статус и даты', {
            'fields': ('status', 'order', 'published_at'),
            'classes': ('collapse',)
        }),
    )
    ...
    def price_display(self, obj):
        ...
        if obj.old_price:
            return format_html('<span style="text-decoration: line-through;">{} ₽</span> → <b>{} ₽</b>', obj.old_price,
                               obj.price)
        return f'{obj.price} ₽'
...
class LessonAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ('Информация об уроке', {
            'fields': ('course', 'title', 'order', 'duration_minutes')
        }),
        ('Контент', {
            'fields': ('video_url', 'content', 'is_free_preview')
        }),
    )
